// DiskID.cpp : Defines the entry point for the DLL application.
//
...
BOOL DoIDENTIFY (HANDLE, PSENDCMDINPARAMS, PSENDCMDOUTPARAMS, BYTE, BYTE,
                 PDWORD);
//int ReadPhysicalDriveInNT (int idrive, void* drivedata, int datalen);
...
extern "C"  int  __stdcall ReadPhysicalDriveInNT(const int idrive,PDRIVE_INFO_OK buf,int buflen)
{
	int done = 0;
	int drive = 0;
...
extern "C" bool  __stdcall  IsWinNT()
{
...
extern "C" int  __stdcall ReadDrivePortsInWin9X (int idrive, PDRIVE_INFO_OK buf,int buflen)
{
	int done = FALSE;
	int drive = 0;
...
extern "C"  int  __stdcall ReadIdeDriveAsScsiDriveInNT (int controller,PDRIVE_INFO_OK buf,int buflen)
{
   int done = FALSE;
//   int controller = 0;
...
extern "C"  long  __stdcall  getHardDriveComputerID ()
{
   int done = FALSE;
   //char string [1024];
   __int64 id = 0;
...
//   done = ReadPhysicalDriveInNT ();
      ...
      //     the IDE drives
   //if ( ! done) done = ReadIdeDriveAsScsiDriveInNT (0);
