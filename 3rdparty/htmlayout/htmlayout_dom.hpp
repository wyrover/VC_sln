        ...
        namespace dom
        {
            ...
            class callback
            {
                public:
                    ...
                    virtual bool on_element(HELEMENT he) = 0; /* return false
                        to continue enumeration*/
                    #if !defined(_MSC_VER) || _MSC_VER > 1200
                        virtual ~callback(){}
                        // this line causing internal compiler error in VC6
                    #endif
            };
            ...
            class attribute_accessor
            {
                public:
                    ...
                    {
                            el_ = (HTMLayout_UseElement(el) == HLDOM_OK) ? el :
                                0;
                            key_ = key;
                    }
                    ...
                    {
                            HLDOM_RESULT r = HTMLayoutSetAttributeByName(el_,
                                key_, value);
                            assert(r == HLDOM_OK);
                            r;
                            return  *this;
                    ...
                    {
                            return wcscmp(this->operator const wchar_t *(),
                                rv.operator const wchar_t *()) == 0;
                    }
                    ...
                    {
                            const wchar_t *lpw = 0;
                            if (key_[0] == '-' && (HTMLayoutGetStyleAttribute
                                (el_, key_, &lpw) == HLDOM_OK) && lpw)
                                return lpw;
                            HTMLayoutGetAttributeByName(el_, key_[0] == '-' ?
                                key_ + 1: key_, &lpw);
                            return lpw;
                    }
            ...
            class element
            {
                protected:
                    ...
                    void use(HELEMENT h)
                    {
                            he = (HTMLayout_UseElement(h) == HLDOM_OK) ? h : 0;
                    } void unuse()
                    {
                            if (he)
                ...
                public:
                     ...
                     **/
                    int get_attribute_int(const char *name, int def_val = 0)
                        const
                    {
                     ...
                     **/
                    const wchar_t *attribute(const char *name, const wchar_t
                        *default_value)const
                    {
                            assert(name[0] == '-');
                    ...
                    color attribute(const char *name, color default_value)const
                    {
                            ...
                            if (txt == none)
                                return default_value;
                            return color::parse(aux::chars_of(txt),
                                default_value);
                    }
                     ...
                     **/
                    void set_style_attribute(const char *name, const wchar_t
                        *value)
                    {
                     ...
                     **/
                    static HELEMENT find_element(HWND hHTMLayoutWnd, POINT
                        clientPt)
                    {
                            HELEMENT h = 0;
                    ...
                    void set_event_root()
                    {
                            HTMLayoutSetEventRoot(he, 0);
                    }
                    void set_event_root(element &prev)
                    {
                            HELEMENT phe = 0;
                            HTMLayoutSetEventRoot(he, &phe);
                            prev = phe;
                    }

                    /**Reset event root.
                     **/
                    void reset_event_root()
                    {
                            HTMLayoutSetEventRoot(dom::element(he).root(), 0);
                    }
                    ...
                    inline static BOOL CALLBACK callback_func(HELEMENT he,
                        LPVOID param)
                    {
                            callback *pcall = (callback*)param;
                            return pcall->on_element(he) ? TRUE : FALSE;
                     ...
                     **/
                    inline void select(callback *pcall, const char *tag_name =
                        0, const char *attr_name = 0, const wchar_t *attr_value
                        = 0, int depth = 0)const
                    {
                            HTMLayoutVisitElements(he, tag_name, attr_name,
                                attr_value, callback_func, pcall, depth);
                    }

                    inline HELEMENT select_parent(const char *selectors,
                        // CSS selectors, comma separated list
                    int depth)const
                    {
                            HELEMENT heFound = 0;
                            HLDOM_RESULT r = HTMLayoutSelectParent(he,
                                selectors, depth, &heFound);
                            assert(r == HLDOM_OK);
                            r;
                            return heFound;
                    }

                    inline HELEMENT select_parent(const wchar_t *selectors,
                        // CSS selectors, comma separated list
                    int depth)const
                    {
                            HELEMENT heFound = 0;
                            HLDOM_RESULT r = HTMLayoutSelectParentW(he,
                                selectors, depth, &heFound);
                            assert(r == HLDOM_OK);
                            r;
                            return heFound;
                    ...
                    inline void select_elements(callback *pcall, const char
                        *selectors  // CSS selectors, comma separated list
                    )const
                    {
                            HTMLayoutSelectElements(he, selectors,
                                callback_func, pcall);
                    }

                    inline void select_elements(callback *pcall, const wchar_t
                        *selectors  // CSS selectors, comma separated list
                    )const
                    {
                            HTMLayoutSelectElementsW(he, selectors,
                                callback_func, pcall);
                    }
                    ...
                    void refresh()const
                    {
                            ...
                            ::InvalidateRect(get_element_hwnd(false), &rc,
                                FALSE);
                    }
                     ...
                     **/
                    RECT get_location(unsigned int area = ROOT_RELATIVE |
                        CONTENT_BOX)const
                    {
                            RECT rc =
                            {
                     ...
                     **/
                    void scroll_to_view(bool toTopOfView = false, bool smooth =
                        false)
                    {
                            UINT flags = 0;
                            if (toTopOfView)
                    ...
                    void get_scroll_info(POINT &scroll_pos, RECT &view_rect,
                        SIZE &content_size)
                    {
                            HLDOM_RESULT r = HTMLayoutGetScrollInfo(he,
                                &scroll_pos, &view_rect, &content_size);
                            assert(r == HLDOM_OK);
                            r;
                    }
                    void set_scroll_pos(POINT scroll_pos, bool smooth = true)
                    {
                            HLDOM_RESULT r = HTMLayoutSetScrollPos(he,
                                scroll_pos, BOOL(smooth));
                            assert(r == HLDOM_OK);
                            r;
                    }
                    ...
                    {
                            HLDOM_RESULT r = HTMLayoutGetElementIntrinsicWidths
                                (he, &min_width, &max_width);
                            assert(r == HLDOM_OK);
                            r;
                    }
                    ...
                    {
                            HLDOM_RESULT r = HTMLayoutGetElementIntrinsicHeight
                                (he, for_width, &min_height);
                            assert(r == HLDOM_OK);
                            r;
                    }
                    ...
                    {
                            HWND hwnd = 0;
                            HTMLayoutGetElementHwnd(he, &hwnd, root_window ?
                                TRUE : FALSE);
                            return hwnd;
                    }
                     ...
                     **/
                    void set_html(const unsigned char *html, size_t html_length,
                        int where = SIH_REPLACE_CONTENT)
                    {
                            ...
                            {
                                    HLDOM_RESULT r = HTMLayoutSetElementHtml(he,
                                        html, DWORD(html_length), where);
                                    assert(r == HLDOM_OK);
                                    r;
                            }
                    }
                    ...
                    static void CALLBACK _writer_a(LPCBYTE utf8, UINT
                        utf8_length, LPVOID pstdstr)
                    {
                    ...
                    {
                            json::astring str;
                            HLDOM_RESULT r = HTMLayoutGetElementHtmlCB(he,
                                outer, &_writer_a, &str);
                            assert(r == HLDOM_OK);
                            r;
                            return str;
                    ...
                    static void CALLBACK _writer_w(const wchar_t *text,
                        unsigned int text_length, void *pstdstr)
                    {
                    ...
                    {
                            json::string str;
                            HLDOM_RESULT r = HTMLayoutGetElementInnerTextCB(he,
                                &_writer_w, &str);
                            assert(r == HLDOM_OK);
                            r;
                            return str;
                    ...
                    {
                            HLDOM_RESULT r = HTMLayoutSetElementInnerText16(he,
                                utf16, (UINT)utf16_length);
                            assert(r == HLDOM_OK);
                            r;
                    }
                    ...
                    inline int vsnprintf(wchar_t *buffer, size_t count, const
                        wchar_t *format, va_list argptr)const
                    {
                            return _vsnwprintf(buffer, count, format, argptr);
                    }

                    inline int vsnprintf(char *buffer, size_t count, const char
                        *format, va_list argptr)const
                    {
                    ...
                    template <class char_t> HELEMENT find_first(const char_t
                        *selector, ...)const
                    {
                            char_t buffer[2049];
                            buffer[0] = 0;
                            ...
                            vsnprintf(buffer, 2048, selector, args);
                            va_end(args);
                            find_first_callback find_first;
                            select_elements(&find_first, buffer);
                                // find first element satisfying given CSS selector
                            return find_first.hfound;
                    }

                    // Find all child elements matching the selector
                    // :root is the element itself
                    template <class char_t> void find_all(callback *cb, const
                        char_t *selector, ...)const
                    {
                            char_t buffer[2049];
                            buffer[0] = 0;
                            va_list args;
                            ...
                            vsnprintf(buffer, 2048, selector, args);
                            va_end(args);
                            select_elements(cb, buffer);
                                // find all elements satisfying given CSS selector
                    ...
                    class find_all_callback: public callback
                    {
                        ...
                        public:
                            find_all_callback(std::vector < dom::element >
                                &_all): all(&_all){}
                            ~find_all_callback(){}
                            bool on_element(HELEMENT he)
                            {
                                    all->push_back(dom::element(he));
                                    return false; /*continue enumeration*/
                            }
                    };

                    template <class char_t> void find_all(std::vector<dom
                        ::element>  &all, const char_t *selector, ...)const
                    {
                            char_t buffer[2049];
                            buffer[0] = 0;
                            va_list args;
                    ...
                    template <class char_t> HELEMENT find_nearest_parent(const
                        char_t *selector, ...)const
                    {
                            char_t buffer[2049];
                            buffer[0] = 0;
                    ...
                    // :root is document root object
                    template <class char_t> bool test(const char_t *selector,
                        ...)const
                    {
                            char_t buffer[2049];
                            buffer[0] = 0;
                    ...
                    {
                            UINT state = 0;
                            HLDOM_RESULT r = HTMLayoutGetElementState(he,
                                &state);
                            assert(r == HLDOM_OK);
                            r;
                     ...
                     **/
                    bool get_state( /*ELEMENT_STATE_BITS*/unsigned int bits)
                        const
                    {
                            UINT state = 0;
                            HLDOM_RESULT r = HTMLayoutGetElementState(he,
                     ...
                     **/
                    void set_state( /*ELEMENT_STATE_BITS*/unsigned int
                        bitsToSet,  /*ELEMENT_STATE_BITS*/unsigned int
                        bitsToClear = 0, bool update = true)
                    {
                            HLDOM_RESULT r = HTMLayoutSetElementState(he,
                                bitsToSet, bitsToClear, update ? TRUE : FALSE);
                            assert(r == HLDOM_OK);
                            r;
                    }

                    void toggle_state( /*ELEMENT_STATE_BITS*/unsigned int
                        bitsToSet, bool onOff)
                    {
                            HLDOM_RESULT r = HTMLayoutSetElementState(he, onOff
                                ? bitsToSet : 0, onOff ? 0 : bitsToSet, TRUE);
                            assert(r == HLDOM_OK);
                            r;
                    }
                    ...
                    {
                            HLDOM_RESULT r = HTMLayoutSetTimerEx(he, millis,
                                tid);
                            assert(r == HLDOM_OK);
                            r;
                    }
                     ...
                     **/
                    static element create(const char *tagname, const wchar_t
                        *text = 0)
                    {
                            element e(0);
                            HLDOM_RESULT r = HTMLayoutCreateElement(tagname,
                                text, &e.he);
                    ...
                    {
                            HLDOM_RESULT r = HTMLayoutInsertElement(e.he, this
                                ->he, index);
                            assert(r == HLDOM_OK);
                            r;
                    }
                     ...
                     **/
                    bool send_event(unsigned int event_code, UINT_PTR reason =
                        0, HELEMENT heSource = 0)
                    {
                            BOOL handled = FALSE;
                            HLDOM_RESULT r = HTMLayoutSendEvent(he, event_code,
                                heSource ? heSource : he, reason, &handled);
                            assert(r == HLDOM_OK);
                            r;
                            return handled != 0;
                    }
                     ...
                     **/
                    void post_event(unsigned int event_code, unsigned int
                        reason = 0, HELEMENT heSource = 0)
                    {
                            HLDOM_RESULT r = HTMLayoutPostEvent(he, event_code,
                                heSource ? heSource : he, reason);
                            assert(r == HLDOM_OK);
                            r;
                    }
                    ...
                    {
                            HLDOM_RESULT r = HTMLayoutMoveElement(he,
                                x_view_rel, y_view_rel);
                            assert(r == HLDOM_OK);
                            r;
                    }
                    void move(int x_view_rel, int y_view_rel, int width, int
                        height)
                    {
                            HLDOM_RESULT r = HTMLayoutMoveElementEx(he,
                                x_view_rel, y_view_rel, width, height);
                            assert(r == HLDOM_OK);
                            r;
                    }
                    ...
                    {
                            HELEMENT hItem = 0;
                            HTMLayoutTrackPopupAt(he, root_pt, animate ? 1 : 0,
                                &hItem);
                            return hItem;
                    }

                    // 'placement' here is a point of the popup that corresponds to root_pt
                    HELEMENT track_popup(uint placement, POINT root_pt, bool
                        animate = false)
                    {
                            HELEMENT hItem = 0;
                            HTMLayoutTrackPopupAt(he, root_pt, (placement << 16)
                                | (animate ? 1 : 0), &hItem);
                            return hItem;
                    }
                    ...
                    void show_popup(uint placement, POINT root_pt, bool animate
                        = false)
                    {
                            HTMLayoutShowPopupAt(he, root_pt, (placement << 16)
                                | (animate ? 1 : 0));
                    }
                    ...
                    bool call_behavior_method(METHOD_PARAMS *p)
                    {
                            if (!is_valid())
                                return false;
                            return ::HTMLayoutCallBehaviorMethod(he, p) ==
                                HLDOM_OK;
                    }

                    json::value xcall(const char *name, json::value p0 = json
                        ::value(), json::value p1 = json::value(), json::value
                        p2 = json::value(), json::value p3 = json::value(),
                        json::value p4 = json::value(), json::value p5 = json
                        ::value(), json::value p6 = json::value(), json::value
                        p7 = json::value())
                    {
                    ...
                    {
                            HLDOM_RESULT r = HTMLayoutRequestElementData(he,
                                url, HLRT_DATA_HTML, initiator);
                            assert(r == HLDOM_OK);
                            r;
                    }
                    ...
                    struct comparator
                    {
                            comparator(){}
                            virtual ~comparator(){}
                            virtual int compare(const htmlayout::dom::element
                                &e1, const htmlayout::dom::element &e2) = 0;


                            static INT CALLBACK scmp(HELEMENT he1, HELEMENT he2,
                                LPVOID param)
                            {
                                    htmlayout::dom::element::comparator *self =
                                        static_cast < htmlayout::dom::element
                                        ::comparator * > (param);
                    ...
                    void sort(comparator &cmp, int start = 0, int end =  - 1)
                    {
                            ...
                            HLDOM_RESULT r = HTMLayoutSortElements(he, start,
                                end, &comparator::scmp, &cmp);
                            assert(r == HLDOM_OK);
                            r;
                    }
            ...
            {
                    static void CALLBACK _finalizer(HTMLayoutElementExpando
                        *pexp, HELEMENT he)
                    {
                            static_cast < expando * > (pexp)->finalize();
                } protected:
            ...
            {
                HLDOM_RESULT r = HTMLayoutElementSetExpando(he, static_cast <
                    HTMLayoutElementExpando * > (exp));
                assert(r == HLDOM_OK);
                r;
            }
            ...
            class text_selection_params: public TEXT_SELECTION_PARAMS
            {
                    ...
                    text_selection_params(): TEXT_SELECTION_PARAMS(false), _wos
                        (0){}
                public:
                    static BOOL CALLBACK ctl(text_selection_params *prms, UINT
                        data)
                    {
                            prms->_wos->push(wchar_t(data));
                            return 1;
                    } text_selection_params(pod::buffer < wchar_t >  &wos):
                        TEXT_SELECTION_PARAMS(false), _wos(&wos)
                    {
                            outs = (OutputStreamProc*)ctl;
                    }
            };
            class html_selection_params: public TEXT_SELECTION_PARAMS
            {
                    pod::buffer < byte >  *_bos;
                    html_selection_params(): TEXT_SELECTION_PARAMS(true), _bos
                        (0){}
                public:
                    static BOOL CALLBACK ctl(html_selection_params *prms, UINT
                        data)
                    {
                            prms->_bos->push(byte(data));
                            return 1;
                    } html_selection_params(pod::buffer < byte >  &bos):
                        TEXT_SELECTION_PARAMS(true), _bos(&bos)
                    {
                            outs = (OutputStreamProc*)ctl;
                    }
            };
            ...
            class editbox: public element
            {
                ...
                public:
                    ...
                    std::wstring text_value()const
                    {
                            ...
                            if (const_cast < editbox * > (this)
                                ->call_behavior_method(&sp) && sp.text &&
                                sp.length)
                            {
            ...
            class scrollbar: public element
            {
                public:
                    ...
                    void get_values(int &val, int &min_val, int &max_val, int
                        &page_val, int &step_val)
                    {
                            ...
                            step_val = sbvp.step_value;
                                // step increment (arrow button click)
                    }

                    void set_values(int val, int min_val, int max_val, int
                        page_val, int step_val)
                    {
            ...
            struct enumerator
            {
                ...
                {
                    HLDOM_RESULT r = HTMLayoutEnumerate(what, &_cb, this,
                        forward);
                    assert(r == HLDOM_OK);
                    r;
                }

                virtual bool on_element_head(HELEMENT)
                {
                    return false;
                }
                virtual bool on_element_tail(HELEMENT)
                {
                    return false;
                }
                virtual bool on_char(HELEMENT text, int pos, WCHAR code)
                {
                    return false;
                }

                inline static BOOL CALLBACK _cb(LPVOID p, HELEMENT he, int pos,
                    int postype, WCHAR code)
                {
                    enumerator *pe = (enumerator*)p;
                    switch (postype)
                    {
                        case 0:
                            // - he element head position.
                            return pe->on_element_head(he);
                        case 1:
                            // - he element tail position.
