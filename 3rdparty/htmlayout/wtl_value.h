...
//
    ...
    namespace WTL
    {
        ...
        struct UnitsCtl
        {
                ...
                CValueT < UnitsCtl > Convert(const CValueT < UnitsCtl >  &src)
                    const;
                // default units used for costruction
                static UnitsCtl defUnits()
                {
                        static UnitsCtl dv;
                        return dv;
                }
        ...
        {
            public:
                typedef enum _CValueType
                {
                        V_UNDEFINED, V_BOOL, V_INT, V_REAL, V_STRING, V_WSTRING
                } CValueType;
            ...
            protected:
                struct stringData
                {
                        ...
                        static stringData *AllocateA(LPCSTR lpsz)
                        {
                                ...
                                stringData *pd = (stringData*)new BYTE[sizeof
                                    (stringData) + sizeof(CHAR)*(sz + 1)];
            ...
            public:
                CValueT(): vtType(V_UNDEFINED)
                {
                        uData.lVal = 0;
                }
                CValueT(bool v, UNITCTL units = UNITCTL::defUnits()): vtType
                    (V_UNDEFINED)
                {
                        Set(v, units);
                }
                CValueT(int v, UNITCTL units = UNITCTL::defUnits()): vtType
                    (V_UNDEFINED)
                {
                        Set(v, units);
                }
                CValueT(double v, UNITCTL units = UNITCTL::defUnits()): vtType
                    (V_UNDEFINED)
                {
                        Set(v, units);
                }
                CValueT(LPCSTR lpsz, UNITCTL units = UNITCTL::defUnits()):
                    vtType(V_UNDEFINED)
                {
                        Set(lpsz, units);
                }
                CValueT(LPCWSTR lpsz, UNITCTL units = UNITCTL::defUnits()):
                    vtType(V_UNDEFINED)
                {
                        Set(lpsz, units);
                }
                CValueT(const CValueT &src, UNITCTL units = UNITCTL::defUnits())
                    : vtType(V_UNDEFINED)
                {
                        Set(src);
                        ctlUnits = units;
                ...
                void Set(LPCSTR lpsz, UNITCTL units = UNITCTL::defUnits())
                {
                        Clear();
                        if (lpsz && lpsz[0])
                        {
                                uData.sVal = stringData::AllocateA(lpsz);
                                vtType = V_STRING;
                                ctlUnits = units;
                        }
                }
                void Set(LPCWSTR lpsz, UNITCTL units = UNITCTL::defUnits())
                {
                        Clear();
                        if (lpsz && lpsz[0])
                        {
                                uData.sVal = stringData::AllocateW(lpsz);
                                vtType = V_WSTRING;
                                ctlUnits = units;
                        }
                }
                void Set(int i, UNITCTL units = UNITCTL::defUnits())
                {
                        Clear();
                        uData.iVal = i;
                        vtType = V_INT;
                ...
                void Set(bool b, UNITCTL units = UNITCTL::defUnits())
                {
                        Clear();
                        uData.iVal = (int)b;
                        vtType = V_BOOL;
                ...
                void Set(double r, UNITCTL units = UNITCTL::defUnits())
                {
                        Clear();
                        uData.rVal = r;
                        vtType = V_REAL;
                ...
                {
                        return (vtType == V_UNDEFINED);
                }
                bool IsText()const
                {
                        return (vtType == V_STRING) || (vtType == V_WSTRING);
                }
                bool IsInt()const
                {
                        return (vtType == V_INT);
                ...
                bool IsReal()const
                {
                        return (vtType == V_REAL);
                }
                bool IsBool()const
                {
                        return (vtType == V_BOOL);
                }
                bool IsSTR()const
                {
                        return (vtType == V_STRING);
                }
                bool IsWSTR()const
                {
                        return (vtType == V_WSTRING);
                }

                CValueT &operator = (LPCWSTR lps)
                {
                        Set(lps);
                        return  *this;
                }
                CValueT &operator = (LPCSTR lps)
                {
                        Set(lps);
                        return  *this;
                }
                CValueT &operator = (int v)
                {
                        Set(v);
                        return  *this;
                }
                CValueT &operator = (double v)
                {
                        Set(v);
                        return  *this;
                }
                CValueT &operator = (bool v)
                {
                        Set(v);
                        return  *this;
                }
                ...
                LPCWSTR GetWSTR()const
                {
                        ...
                        return (vtType == V_WSTRING) ? (LPCWSTR)uData.sVal
                            ->data(): L "";
                }
                ...
                {
                        return ctlUnits.ToString(*this);
                }
                void FromString(LPCTSTR str)
                {
                        Set(ctlUnits.FromString(str));
                }
        ...
        inline CValueT < UnitsCtl > UnitsCtl::FromString(LPCTSTR lpsz)const
        {
            ...
            if (_tcsicmp(lpsz, TEXT("true")) == 0 || _tcsicmp(lpsz, TEXT("yes"))
                == 0)
                return CValueT < UnitsCtl > (true);
            if (_tcsicmp(lpsz, TEXT("false")) == 0 || _tcsicmp(lpsz, TEXT("no"))
                == 0)
        ...
        inline CString UnitsCtl::ToString(const CValueT < UnitsCtl >  &v)const
        {
            ...
            switch (v.Type())
            {
                case CValueT < UnitsCtl > ::V_UNDEFINED: out = TEXT(
                    "{undefined}");
                break;
                case CValueT < UnitsCtl > ::V_BOOL: out = v.GetBool() ? TEXT(
                    "true"): TEXT("false");
                break;
                case CValueT < UnitsCtl > ::V_INT: out.Format(TEXT("%d"),
                    v.GetInt());
                break;
                case CValueT < UnitsCtl > ::V_REAL: out.Format(TEXT("%f"),
                    v.GetReal());
                break;
                case CValueT < UnitsCtl > ::V_STRING: case CValueT < UnitsCtl >
                    ::V_WSTRING: return v.GetText();
            }
            return out;
        }

        inline CValueT < UnitsCtl > UnitsCtl::Convert(const CValueT < UnitsCtl
            >  &src)const
        {
