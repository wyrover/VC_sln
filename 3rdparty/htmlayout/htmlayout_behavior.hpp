 ...
 */
    ...
    #include "htmlayout_behavior.h"
        ...
        // appears as really bad number indeed
    ...
    namespace htmlayout
    {
        ...
        struct event_handler
        {
            ...
            {
                return on_key(he, params.target, params.cmd, params.key_code,
                    params.alt_state);
            }
            ...
            {
                return on_script_call(he, params.method_name, params.argc,
                    params.argv, params.retval);
            }


            // notification events from builtin behaviors - synthesized events: BUTTON_CLICK, VALUE_CHANGED
            // see enum BEHAVIOR_EVENTS
            virtual BOOL handle_event(HELEMENT he, BEHAVIOR_EVENT_PARAMS
                &params)
            {
                return on_event(he, params.heTarget, (BEHAVIOR_EVENTS)
                    params.cmd, params.reason);
            }

            // notification event: data requested by HTMLayoutRequestData just delivered
            virtual BOOL handle_data_arrived(HELEMENT he, DATA_ARRIVED_PARAMS
                &params)
            {
                return on_data_arrived(he, params.initiator, params.data,
                    params.dataSize, params.dataType);
            }
            ...
            virtual BOOL on_method_call(HELEMENT he, UINT methodID,
            ...
            // notification events from builtin behaviors - synthesized events: BUTTON_CLICK, VALUE_CHANGED
            // see enum BEHAVIOR_EVENTS
            virtual BOOL on_event(HELEMENT he, HELEMENT target, BEHAVIOR_EVENTS
                type, UINT_PTR reason)
            {
                return FALSE;
            }

            // notification event: data requested by HTMLayoutRequestData just delivered
            virtual BOOL on_data_arrived(HELEMENT he, HELEMENT initiator,
            ...
            static BOOL CALLBACK element_proc(LPVOID tag, HELEMENT he, UINT
                evtg, LPVOID prms)
            {
                ...
                switch (evtg)
                {
                    case HANDLE_INITIALIZATION:
                        {
                            INITIALIZATION_PARAMS *p = (INITIALIZATION_PARAMS*)
                                prms;
                    ...
                    case HANDLE_BEHAVIOR_EVENT:
                        {
                            BEHAVIOR_EVENT_PARAMS *p = (BEHAVIOR_EVENT_PARAMS*)
                                prms;
                    ...
                    case HANDLE_EXCHANGE:
                        {
                            EXCHANGE_PARAMS *p = (EXCHANGE_PARAMS*)prms;
        ...
        inline void attach_event_handler(HELEMENT he, event_handler
            *p_event_handler, UINT subscription = HANDLE_ALL)
        {
            HTMLayoutAttachEventHandlerEx(he, &event_handler::element_proc,
                p_event_handler, subscription);
        }

        inline void detach_event_handler(HELEMENT he, event_handler
            *p_event_handler)
        {
            HTMLayoutDetachEventHandler(he, &event_handler::element_proc,
                p_event_handler);
        }

        // "manually" attach event_handler proc to the window
        inline void attach_event_handler(HWND hwndLayout, event_handler
            *p_event_handler, UINT subscription = HANDLE_ALL)
        {
            HTMLayoutWindowAttachEventHandler(hwndLayout, &event_handler
                ::element_proc, p_event_handler, subscription);
        }
        inline void detach_event_handler(HWND hwndLayout, event_handler
            *p_event_handler)
        {
            HTMLayoutWindowDetachEventHandler(hwndLayout, &event_handler
                ::element_proc, p_event_handler);
        }
        ...
        //
            ...
            #include "behavior_impl.hpp"
                // provide your own implementation with ctor and handle() method as defined below
            ...
            struct behavior: event_handler
            {
                ...
                behavior(UINT subsriptions, const char *external_name): next(0),
                    name(external_name), event_handler(subsriptions)
                {
                ...
                {
                    htmlayout::event_handler *pb = htmlayout::behavior::find
                        (lpab->behaviorName, lpab->element);
                    if (pb)
                    {
                        lpab->elementTag = pb;
                        lpab->elementProc = htmlayout::event_handler
                            ::element_proc;
