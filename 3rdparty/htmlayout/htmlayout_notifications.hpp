 ...
 **/
    ...
    namespace htmlayout
    {
         ...
         **/
        template <typename BASE> struct notification_handler
        {
            ...
            {
                HTMPrintSetCallback(printex, callback, static_cast < BASE * >
                    (this));
            }


            // HTMLayout callback
            static LRESULT CALLBACK callback(UINT uMsg, WPARAM wParam, LPARAM
            ...
            LRESULT handle_notification(UINT uMsg, WPARAM wParam, LPARAM lParam)
            {
                ...
                switch (((NMHDR*)lParam)->code)
                {
                    case HLN_CREATE_CONTROL:
                        return on_create_control((LPNMHL_CREATE_CONTROL)lParam);
                    case HLN_CONTROL_CREATED:
                        return on_control_created((LPNMHL_CREATE_CONTROL)lParam)
                            ;
                    case HLN_DESTROY_CONTROL:
                        return on_destroy_control((LPNMHL_DESTROY_CONTROL)
                            lParam);
                    case HLN_LOAD_DATA:
                        return on_load_data((LPNMHL_LOAD_DATA)lParam);
                    case HLN_DATA_LOADED:
                        return on_data_loaded((LPNMHL_DATA_LOADED)lParam);
                    case HLN_DOCUMENT_COMPLETE:
                        return on_document_complete();
                    case HLN_DOCUMENT_LOADED:
                        return on_document_loaded();
                    case HLN_ATTACH_BEHAVIOR:
                        return on_attach_behavior((LPNMHL_ATTACH_BEHAVIOR)
                            lParam);
                    case HLN_DIALOG_CREATED:
                        return on_dialog_created((NMHDR*)lParam);
                    case HLN_DIALOG_CLOSE_RQ:
                        return on_dialog_close((LPNMHL_DIALOG_CLOSE_RQ)lParam);
                }
                return 0;
            }
            ...
            virtual LRESULT on_control_created(LPNMHL_CREATE_CONTROL pnmcc)
            {
                return 0;
            }
            virtual LRESULT on_destroy_control(LPNMHL_DESTROY_CONTROL pnmhl)
            {
                return 0;
            }
            ...
            virtual LRESULT on_load_data(LPNMHL_LOAD_DATA pnmld)
            {
                ...
                if (load_resource_data(pnmld->uri, pb, cb))
                    ::HTMLayoutDataReady(pnmld->hdr.hwndFrom, pnmld->uri, pb,
                        cb);
                return LOAD_OK;
            }
            ...
            virtual LRESULT on_dialog_created(NMHDR*)
            {
                return 0;
            }
            virtual LRESULT on_dialog_close(LPNMHL_DIALOG_CLOSE_RQ pnmld)
            {
                return 0;
            }
        ...
        inline bool load_html_resource(UINT resId, PBYTE &pb, DWORD &cb)
        {
            HRSRC hrsrc = ::FindResource(0, MAKEINTRESOURCE(resId),
                MAKEINTRESOURCE(23));
            if (!hrsrc)
                return FALSE;
            // resource not found here - proceed with default loader
