 ...
 **/
     ...
     **/
    typedef LRESULT CALLBACK HTMLAYOUT_NOTIFY(UINT uMsg, WPARAM wParam, LPARAM
        lParam, LPVOID vParam);
    ...
    typedef struct tagNMHL_CREATE_CONTROL
    {
        ...
        HWND outControlHwnd; /**< [out] HWND of control created or
        #HWND_TRY_DEFAULT or HWND_DISCARD_CREATION.*/
        DWORD reserved1;
        DWORD reserved2;
    ...
    typedef struct tagNMHL_LOAD_DATA
    {
        ...
        HELEMENT principal; /**< [in] element requested download, in case of
        context_menu:url( menu-url )
    ...
    typedef struct tagNMHL_DATA_LOADED
    {
        ...
        DWORD dataSize; /**< [in] loaded data size (in bytes). dataSize == 0 -
        incompatible data type, e.g. requested image but HTML returned */
        UINT dataType; /**< [in] HTMLayoutResourceType */
        UINT status; /**< [in]
    ...
    typedef struct tagNMHL_ATTACH_BEHAVIOR
    {
        ...
        LPCSTR behaviorName; /**< [in] zero terminated string, string appears as
        value of CSS behavior:"???" attribute.*/

        ElementEventProc *elementProc; /**< [out] pointer to ElementEventProc
        function.*/
        LPVOID elementTag; /**< [out] tag value, passed as is into pointer
        ElementEventProc function.*/
        UINT elementEvents; /**< [out] EVENT_GROUPS bit flags, event groups
        elementProc subscribed to. */
    ...
    typedef struct tagNMHL_BEHAVIOR_CHANGED
    {
        ...
        HELEMENT element; /**< [in] target DOM element handle*/
        LPCSTR oldNames; /**< [in] zero terminated string, whitespace separated
        list of old behaviors.*/
        LPCSTR newNames; /**< [in] zero terminated string, whitespace separated
        list of new behaviors that the element just got.*/
    ...
    {
        NMHDR hdr; /**< Default WM_NOTIFY header.*/
        BOOL outCancel; /**< [out] set it to non-zero for canceling close
            request.*/
     ...
     **/
    EXTERN_C BOOL HLAPI HTMLayoutDataReady(HWND hwnd, LPCWSTR uri, LPBYTE data,
        DWORD dataLength);
    ...
    {
        HLRT_DATA_HTML = 0, HLRT_DATA_IMAGE = 1, HLRT_DATA_STYLE = 2,
            HLRT_DATA_CURSOR = 3, HLRT_DATA_SCRIPT = 4,
    };
    ...
    EXTERN_C BOOL HLAPI HTMLayoutDataReadyAsync(HWND hwnd, LPCWSTR uri, LPBYTE
        data, DWORD dataLength, UINT dataType /*HTMLayoutResourceType*/);

    #define WPARAM_DISCARD_BUILTIN_DD_SUPP0RT 0x8000
    ...
    EXTERN_C LRESULT CALLBACK HTMLayoutProc(HWND hwnd, UINT msg, WPARAM wParam,
        LPARAM lParam);

    /**HTMLayout Unicode Window Proc.*/
    EXTERN_C LRESULT CALLBACK HTMLayoutProcW(HWND hwnd, UINT msg, WPARAM wParam,
        LPARAM lParam);


    /**HTMLayout Window Proc without call of DefWindowProc.*/
    EXTERN_C LRESULT HLAPI HTMLayoutProcND(HWND hwnd, UINT msg, WPARAM wParam,
        LPARAM lParam, BOOL *pbHandled);
    ...
    enum HTMLayoutModes
    {
        ...
        HLM_SHOW_SELECTION = 1,  /**< layout manager and renderer + text
            selection
        and WM_COPY.*/
    };
     ...
     **/
    typedef BOOL CALLBACK HTMLAYOUT_CALLBACK_RES(LPCWSTR resourceUri, LPCSTR
        resourceType, LPCBYTE imageData, DWORD imageDataSize);
    typedef BOOL CALLBACK HTMLAYOUT_CALLBACK_RES_EX(LPCWSTR resourceUri, LPCSTR
        resourceType, LPCBYTE imageData, DWORD imageDataSize, LPVOID prm);
     ...
     **/
    EXTERN_C BOOL HLAPI HTMLayoutLoadHtml(HWND hWndHTMLayout, LPCBYTE html,
        UINT htmlSize);
     ...
     * \return \b BOOL, \c TRUE if the text was parsed and loaded successfully, FALSE otherwise.
     **/
    EXTERN_C BOOL HLAPI HTMLayoutLoadHtmlEx(HWND hWndHTMLayout, LPCBYTE html,
    ...
    EXTERN_C VOID HLAPI HTMLayoutSetCallback(HWND hWndHTMLayout,
        LPHTMLAYOUT_NOTIFY cb, LPVOID cbParam);
     ...
     **/
    EXTERN_C LPCBYTE HLAPI HTMLayoutGetSelectedHTML(HWND hWndHTMLayout, LPUINT
        pSize);
     ...
     **/
    EXTERN_C UINT HLAPI HTMLayoutEnumResourcesEx(HWND hWndHTMLayout,
        HTMLAYOUT_CALLBACK_RES_EX *cb, LPVOID cbPrm);
     ...
     *
     **/
    typedef void CALLBACK HTMLAYOUT_DATA_WRITER(LPCWSTR uri, UINT dataType,
        LPCBYTE data, UINT dataLength);
    typedef BOOL CALLBACK HTMLAYOUT_DATA_LOADER(LPCWSTR uri, UINT dataType,
        HTMLAYOUT_DATA_WRITER *pDataWriter);

    EXTERN_C BOOL HLAPI HTMLayoutSetDataLoader(HTMLAYOUT_DATA_LOADER
        *pDataLoader);
    ...
    enum ELEMENT_MODEL
    {
        ...
        INLINE_TEXT_ELEMENT = 1,
        // inline text, can contain text, example: <em>. Will get style display:inline.
        INLINE_BLOCK_ELEMENT = 2,
        // inline element, contains blocks inside, example: <select>. Will get style display:inline-block.
        BLOCK_TEXT_ELEMENT = 3,
        // block of text, can contain text, example: <p>. Will get styles display:block; width:*.
        BLOCK_BLOCK_ELEMENT = 4,
        // block of blocks, contains blocks inside, example: <div>. Will get style display:block.; width:*.
    };
     ...
     *
     **/
    EXTERN_C BOOL HLAPI HTMLayoutDeclareElementType(LPCSTR name, UINT
    ...
    EXTERN_C BOOL HLAPI HTMLayoutSetCSS(HWND hWndHTMLayout, LPCBYTE utf8, UINT
        numBytes, LPCWSTR baseUrl, LPCWSTR mediaType);
    ...
    EXTERN_C BOOL HLAPI HTMLayoutSetMediaType(HWND hWndHTMLayout, LPCWSTR
        mediaType);
    ...
    EXTERN_C BOOL HLAPI HTMLayoutSetMediaVars(HWND hWndHTMLayout, const
        VALUE*mediaVars);
     ...
     *
     **/
    EXTERN_C BOOL HLAPI HTMLayoutSetHttpHeaders(HWND hWndHTMLayout, LPCSTR
        httpHeaders, UINT httpHeadersLength);
    ...
    enum HTMLAYOUT_OPTIONS
    {
        ...
        // value: 0 - multimedia timer thread, 1 - GUI thread (timing is not so accurate).
    ...
    EXTERN_C BOOL HLAPI HTMLayoutSetOption(HWND hWndHTMLayout, UINT option,
        UINT value);
     ...
     *
     **/
    EXTERN_C BOOL HLAPI HTMLayoutRender(HWND hWndHTMLayout, HBITMAP hBmp, RECT
        area);
    ...
    EXTERN_C UINT HLAPI HTMLayoutUrlEscape(LPCWSTR text, BOOL spaceToPlus,
        LPSTR buffer, UINT bufferLength);
     ...
     *
     **/
    EXTERN_C UINT HLAPI HTMLayoutUrlUnescape(LPCSTR url, LPWSTR buffer, UINT
        bufferLength);
     ...
     *
     **/
    EXTERN_C INT_PTR HLAPI HTMLayoutDialog(HWND hWndParent, POINT position, INT
        alignment, UINT style, UINT styleEx, LPHTMLAYOUT_NOTIFY
        notificationCallback, LPELEMENT_EVENT_PROC eventsCallback, LPVOID
        callbackParam, LPCBYTE html, UINT htmlLength);
    ...
    EXTERN_C VOID HLAPI HTMLayoutSetupDebugOutput(LPVOID param,
