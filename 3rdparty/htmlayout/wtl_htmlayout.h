...
//
    ...
    namespace WTL
    {
        ...
        bool GetHtmlResource(LPCTSTR pszName,  /*out*/PBYTE &pb,  /*out*/DWORD
            &cb, HMODULE hModule = NULL);


        /////////////////////////////////////////////////////////////////////////////
        // CHTMLayoutCtrl - client side for a HTMLayout control

        template <class TBase> class CHTMLayoutCtrlT: public TBase
        {
            public:
                ...
                HWND CreateLayout(HWND hWndParent, _U_RECT rect = NULL, LPCTSTR
                    szWindowName = NULL, DWORD dwStyle = 0, DWORD dwExStyle = 0,
                    _U_MENUorID MenuOrID = 0U, LPVOID lpCreateParam = NULL)
                {
                        CWindow::Create(GetWndClassName(), hWndParent,
                            rect.m_lpRect, szWindowName, dwStyle | WS_CHILD,
                ...
                bool LoadHtml(LPCBYTE pb, DWORD nBytes, LPCWSTR baseUrl = 0)
                {
                        ...
                        return (baseUrl ? ::HTMLayoutLoadHtmlEx(m_hWnd, pb,
                            nBytes, baseUrl): ::HTMLayoutLoadHtml(m_hWnd, pb,
                            nBytes)) != 0;
                }
                ...
                static inline void AppendBytes(CSimpleValArray < BYTE >  &buf,
                ...
                bool OpenFile(LPCTSTR lpszFilePath)
                {
                        ...
                        SetupMediaType();
                        USES_CONVERSION;
                        return ::HTMLayoutLoadFile(m_hWnd, T2W(const_cast <
                            LPTSTR > (lpszFilePath))) == TRUE;
                }
                ...
                {
                        CRect rc(0, 0, 0, 0);
                        AdjustWindowRectEx(&rc, GetWindowLong(GWL_STYLE),
                            GetMenu() != NULL, GetWindowLong(GWL_EXSTYLE));
                ...
                int GetWindowHeight(int windowWidth)
                {
                        ...
                        if (clientWidth < (int)GetDocumentMinWidth())
                        // horz scrollbar will appear, add its height
                ...
                {
                        ATLASSERT(::IsWindow(m_hWnd));
                        return ::HTMLayoutClipboardCopy(m_hWnd) != 0;
                            // true is selection exist
                }
                ...
                bool CALLBACK HTMLayoutDataReady(LPCWSTR uri, LPBYTE data,
                    DWORD dataLength)
                {
                        return ::HTMLayoutDataReady(m_hWnd, uri, data,
                            dataLength) != FALSE;
                }
                ...
                LRESULT CALLBACK HTMLayoutProc(UINT msg, WPARAM wParam, LPARAM
                    lParam)
                {
                ...
                HWND GetDlgItemByName(LPCSTR nameTemplate, int index = 0)
                {
                        ...
                        htmlayout::dom::element root = htmlayout::dom::element
                            ::root_element(m_hWnd);
                ...
                void SetDlgItemValues(const CSimpleMap < CString, CValue >
                    &bag);
                // </OBSOLETE!>
                ...
                void SetupMediaType()
                {
                            ...
                            ::SystemParametersInfo(SPI_GETHIGHCONTRAST, 0, &hc,
                                0);
                            LPCWSTR mediaType = hc.dwFlags &HCF_HIGHCONTRASTON
                                ? L"contrast-screen": L"screen";
            ...
            private:
                ...
                struct findItemHWndByName: htmlayout::dom::callback
                {
                        ...
                        int nIndexRequested;
                        int nIndex;
                        findItemHWndByName(int index = 0): hWndFound(0),
                            nIndexRequested(index), nIndex(0){}
        ...
        typedef CWinTraits < WS_CHILD | WS_VISIBLE | WS_CLIPCHILDREN |
            WS_CLIPSIBLINGS, WS_EX_CLIENTEDGE > CHTMLayoutWinTraits;
        ...
        inline bool GetHtmlResource(LPCTSTR pszName,  /*out*/PBYTE &pb,
            /*out*/DWORD &cb, HMODULE hModule)
        {
            ...
            HRSRC hrsrc = ::FindResource(hModule, pszName, MAKEINTRESOURCE
                (RT_HTML));
            ...
            if (!pb)
                return false;
            cb = ::SizeofResource(hModule, hrsrc);
            if (!cb)
                return false;
