...
//
    ...
    namespace WTL
    {
        ...
        template <class TBase> inline CValue CHTMLayoutCtrlT<TBase>
            ::GetDlgItemValue(HWND hwndControl)
        {
                ...
                if (type.IsEmpty() || eq(type, TEXT("text")) || eq(type, TEXT(
                    "password")) || eq(type, TEXT("textarea")))
                {
                        WINDOW_TEXT_AS_VALUE: int length =
                            ::GetWindowTextLength(hwndControl) + 1;
                        if (length> 1)
                        {
                                LPTSTR buf = (LPTSTR)_alloca(length *sizeof
                                    (TCHAR));
                ...
                else if (eq(type, TEXT("radio")))
                {
                        ...
                        for (int i = 0; i < count; i++)
                        {
                                ...
                                if (::SendMessage(hwndGroupItem, BM_GETCHECK, 0,
                                    0) == BST_CHECKED)
                                    return CValue(i);
                        }
                        // not set
                ...
                else if (eq(type, TEXT("select")))
                {
                        ...
                        bool isCombo = size.IsUndefined() || (size.IsInt() &&
                            (size.GetInt() <= 1));

                        int selitem = ::SendMessage(hwndControl, isCombo ?
                            CB_GETCURSEL : LB_GETCURSEL, 0, 0);
                        if (selitem == CB_ERR)
                        // no selection at all
                            return CValue();
                ...
                else if (eq(type, TEXT("button")))
                {
                        ...
                        {
                                HWND hwndGroupItem = GetDlgItemByName(name, i);
                                if (::SendMessage(hwndGroupItem, BM_GETSTATE, 0,
                                    0) == BST_PUSHED)
                                    return CValue(i);
                        }
                        // not pressed
                ...
                // try to interpret window text as a value
                    goto WINDOW_TEXT_AS_VALUE;
        }

        template <class TBase> inline void CHTMLayoutCtrlT<TBase>
            ::GetDlgItemValues(CSimpleMap<CString, CValue>  &bag)
        {
        ...
        template <class TBase> inline void CHTMLayoutCtrlT<TBase>
            ::SetDlgItemValue(HWND hwndControl, CValue v)
        {
                ATLASSERT(::IsWindow(m_hWnd));
                ATLASSERT(::IsWindow(hwndControl));
                ATLASSERT(::GetParent(hwndControl) == m_hWnd);

                CString type = GetDlgItemStringAttribute(hwndControl, TEXT(
                    "type"));

                if (type.IsEmpty() || eq(type, TEXT("text")) || eq(type, TEXT(
                    "password")) || eq(type, TEXT("textarea")))
                {
                        WINDOW_TEXT_AS_VALUE: CString s = v.ToString();
                ...
                else if (eq(type, TEXT("radio")))
                {
                        if (v.IsInt())
                        {
                                ...
                                ::SendMessage(hwndGroupItem, BM_SETCHECK,
                                    BST_CHECKED, 0);
                        }
                }
                else if (eq(type, TEXT("checkbox")))
                {
                        if (v.IsInt())
                        {
                                ...
                                {
                                        HWND hwndGroupItem = GetDlgItemByName
                                            (name, i);
                                        ATLASSERT(::IsWindow(hwndGroupItem));
                                        ::SendMessage(hwndGroupItem,
                                            BM_SETCHECK, (bits &bitMask) ?
                ...
                else if (eq(type, TEXT("select")))
                {
                        ...
                        {
                                // list and combo boxes
                                int size = GetDlgItemIntAttribute(hwndControl,
                                    TEXT("size"));
                                bool isCombo = size <= 1;
                                ::SendMessage(hwndControl, isCombo ?
                                    CB_SETCURSEL : LB_SETCURSEL, v.GetInt(), 0);
                        }
                }
                /*
                else if(eq(type,TEXT("htmlarea")))
        ...
        template <class TBase> inline void CHTMLayoutCtrlT<TBase>
            ::SetDlgItemValue(LPCTSTR name, CValue v)
        {
        ...
        template <class TBase> inline void CHTMLayoutCtrlT<TBase>
            ::SetDlgItemValues(const CSimpleMap<CString, CValue>  &bag)
        {
