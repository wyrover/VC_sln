 ...
 */
    ...
    namespace htmlayout
    {
        ...
        struct canvas: event_handler
        {
            ...
            bool default_draw;
            bool redraw;
            canvas(UINT subsriptions,  // what events to handle
            DRAW_EVENTS layer,  // which layer to draw
            bool draw_default = false
                // true if draws this layer completely, false if it needs dafault drawing on that layer
            ): event_handler(subsriptions | HANDLE_DRAW), surface(0), where
                (layer), redraw(false), default_draw(draw_default){}
            ...
            virtual void draw(HELEMENT he, graphics &gx, UINT width, UINT
                height) = 0;


        };

        template <class CANVAS> struct canvas_factory: behavior
        {
                canvas_factory(const char *name): behavior
                    (HANDLE_INITIALIZATION, name){}
