 ...
 */
    ...
    enum EVENT_GROUPS
    {
        ...
        HANDLE_FOCUS = 0x0004,  /** focus events, if this flag is set it also
            means that element it attached to is focusable */
    ...
    enum MOUSE_BUTTONS
    {
        ...
        MIDDLE_MOUSE_BUTTON = 0x04, X1_MOUSE_BUTTON = 0x08, X2_MOUSE_BUTTON =
            0x10,
    };

    enum KEYBOARD_STATES
    {
        CONTROL_KEY_PRESSED = 0x1, SHIFT_KEY_PRESSED = 0x2, ALT_KEY_PRESSED =
            0x4
    };
    ...
    enum MOUSE_EVENTS
    {
        MOUSE_ENTER = 0, MOUSE_LEAVE = 1, MOUSE_MOVE = 2, MOUSE_UP = 3,
            MOUSE_DOWN = 4, MOUSE_DCLICK = 5, MOUSE_WHEEL = 6, MOUSE_TICK = 7,
            // mouse pressed ticks
    ...
    enum FOCUS_EVENTS
    {
        FOCUS_LOST = 0, FOCUS_GOT = 1,
    };

    enum FOCUS_CAUSE
    {
        BY_CODE, BY_MOUSE, BY_KEY_NEXT, BY_KEY_PREV
    };
    ...
    enum SCROLL_EVENTS
    {
        SCROLL_HOME = 0, SCROLL_END, SCROLL_STEP_PLUS, SCROLL_STEP_MINUS,
            SCROLL_PAGE_PLUS, SCROLL_PAGE_MINUS, SCROLL_POS,
    ...
    {
        GESTURE_FLAG_ZOOM = 0x0001, GESTURE_FLAG_ROTATE = 0x0002,
            GESTURE_FLAG_PAN_VERTICAL = 0x0004, GESTURE_FLAG_PAN_HORIZONTAL =
            0x0008, GESTURE_FLAG_TAP1 = 0x0010,  // press & tap
    ...
    {
        X_DRAG_ENTER, X_DRAG_LEAVE, X_DRAG, X_DROP,
    };

    enum EXCHANGE_DATA_TYPE
    {
        EXF_UNDEFINED = 0, EXF_TEXT = 0x01,
            // FETCH_EXCHANGE_DATA will receive UTF8 encoded string - plain text
        EXF_HTML = 0x02,
            // FETCH_EXCHANGE_DATA will receive UTF8 encoded string - html
        EXF_HYPERLINK = 0x04,
            // FETCH_EXCHANGE_DATA will receive UTF8 encoded string with pair url\0caption (null separated)
        EXF_JSON = 0x08,
            // FETCH_EXCHANGE_DATA will receive UTF8 encoded string with JSON literal
        EXF_FILE = 0x10,
            // FETCH_EXCHANGE_DATA will receive UTF8 encoded list of file names separated by nulls
    };
    enum EXCHANGE_COMMANDS
    {
        EXC_NONE = 0, EXC_COPY = 1, EXC_MOVE = 2, EXC_LINK = 4,
    };

    struct EXCHANGE_PARAMS;

    typedef BOOL CALLBACK FETCH_EXCHANGE_DATA(EXCHANGE_PARAMS *params, UINT
        data_type, LPCBYTE *ppDataStart, UINT *pDataLength);
    ...
    struct EXCHANGE_PARAMS
    {
        ...
        FETCH_EXCHANGE_DATA *fetch_data;
            // function to fetch the data of desired type.
    };
    ...
    enum BEHAVIOR_EVENTS
    {
        ...
        REQUEST_TOOLTIP,
            // request tooltip, BEHAVIOR_EVENT_PARAMS.he <- is the tooltip element.
    ...
    enum EVENT_REASON
    {
        BY_MOUSE_CLICK = 0, BY_KEY_CLICK = 1, SYNTHESIZED = 2,
    ...
    struct BEHAVIOR_EVENT_PARAMS
    {
        ...
        UINT reason;
            // EVENT_REASON or EDIT_CHANGED_REASON - UI action causing change.
    ...
    {
        UINT_PTR timerId;
            // timerId that was used to create timer by using HTMLayoutSetTimerEx
    };
    ...
    {
        DO_CLICK = 0, GET_TEXT_VALUE = 1, SET_TEXT_VALUE,
        // p - TEXT_VALUE_PARAMS
        ...
        SCROLL_BAR_GET_VALUE, SCROLL_BAR_SET_VALUE,
    ...
    struct _METHOD_PARAMS
    {
        ...
        struct XCALL_PARAMS: METHOD_PARAMS
        {
            ...
            json::value *argv;
            json::value retval;
            XCALL_PARAMS(LPCSTR name): method_name(name), argc(0), argv(0)
            {
                methodID = XCALL;
            }
        };
        ...
        struct TEXT_VALUE_PARAMS: METHOD_PARAMS
        {
            LPCWSTR text;
            UINT length;
            TEXT_VALUE_PARAMS(bool to_set = false)
            {
                methodID = to_set ? SET_TEXT_VALUE : GET_TEXT_VALUE;
            }
        };
        ...
        struct TEXT_EDIT_SELECTION_PARAMS: METHOD_PARAMS
        {
            ...
            UINT selection_end;
            TEXT_EDIT_SELECTION_PARAMS(bool to_set = false)
            {
                methodID = to_set ? TEXT_EDIT_SET_SELECTION :
                    TEXT_EDIT_GET_SELECTION;
            }
        };
        ...
        struct TEXT_EDIT_REPLACE_SELECTION_PARAMS: METHOD_PARAMS
        {
            LPCWSTR text;
            UINT text_length;
            TEXT_EDIT_REPLACE_SELECTION_PARAMS()
            {
                methodID = TEXT_EDIT_REPLACE_SELECTION;
            }
        };
        ...
        struct TEXT_CARET_POSITION_PARAMS: METHOD_PARAMS
        {
            ...
            TEXT_CARET_POSITION_PARAMS()
            {
                methodID = TEXT_EDIT_GET_CARET_POSITION;
            }
        };
        ...
        struct TEXT_EDIT_CHAR_POS_AT_XY_PARAMS: METHOD_PARAMS
        {
            ...
            HELEMENT he; // out
            int he_pos; // out
            TEXT_EDIT_CHAR_POS_AT_XY_PARAMS()
            {
                methodID = TEXT_EDIT_CHAR_POS_AT_XY;
            }
        };


        struct TEXT_SELECTION_PARAMS;
        typedef BOOL CALLBACK OutputStreamProc(TEXT_SELECTION_PARAMS *params,
            UINT data /* BYTE or WCHAR */);

        struct TEXT_SELECTION_PARAMS: METHOD_PARAMS
        {
            TEXT_SELECTION_PARAMS(bool html)
            {
                methodID = html ? TEXT_EDIT_GET_SELECTION_HTML :
                    TEXT_EDIT_GET_SELECTION_TEXT;
        ...
        struct SCROLLBAR_VALUE_PARAMS: METHOD_PARAMS
        {
            ...
            SCROLLBAR_VALUE_PARAMS(bool to_set = false)
            {
                methodID = to_set ? SCROLL_BAR_SET_VALUE : SCROLL_BAR_GET_VALUE;
            }
        };
