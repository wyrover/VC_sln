...
namespace htmlayout
{
    ...
    struct dropdown: public behavior
    {
            ...
            POINT pt, UINT mouseButtons, UINT keyboardStates)
        {
            ...
            switch (event_type)
            {
                case MOUSE_DOWN:
                    {
                        dom::element popup_el = el.find_first(".popup,popup");
                            // either class or element <popup>
                        if (belongs_to(popup_el, target))
                            return true;

                        if (popup_el.is_valid() && el.is_valid() &&
                            !el.get_state(STATE_OWNS_POPUP))
                        {
                            ...
                            ::HTMLayoutShowPopup(popup_el, el, 2);
                                // show it below
    ...
    {
        // ctor
        popup(): behavior(HANDLE_MOUSE | HANDLE_BEHAVIOR_EVENT | HANDLE_FOCUS,
            "popup"){}
            ...
            POINT pt, UINT mouseButtons, UINT keyboardStates)
        {
            switch (event_type)
            {
                case MOUSE_DOWN:
                    return true;
                case MOUSE_UP | HANDLED: case MOUSE_UP:
            ...
            switch (type)
            {
                case BUTTON_CLICK:
                    #pragma warning( suppress:4063 )
                        // case 'XXX' is not a valid value for switch of enum 'BEHAVIOR_EVENTS'
                case BUTTON_CLICK | HANDLED: case HYPERLINK_CLICK:
                    #pragma warning( suppress:4063 )
                        // case 'XXX' is not a valid value for switch of enum 'BEHAVIOR_EVENTS'
                    case HYPERLINK_CLICK | HANDLED: ::HTMLayoutHidePopup(he);
                    break;
            }
            return false;
        }
