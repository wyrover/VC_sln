...
namespace htmlayout
{
    ...
    struct graphin_clock: public canvas
    {
        ...
        // ctor
        graphin_clock(): canvas(HANDLE_TIMER | HANDLE_MOUSE |
            HANDLE_BEHAVIOR_EVENT, DRAW_CONTENT), pimage(0){}
        ~graphin_clock()
        {
            delete pimage;
        }
        ...
        virtual void draw(HELEMENT he, graphics &gx, UINT width, UINT height)
        {
            ...
            draw_clock_hand(he, gx, width, height, (st.wHour *360.0) / 12.0 +
                st.wMinute / 2.0, 0);
            draw_clock_hand(he, gx, width, height, (st.wMinute *360.0) / 60.0,
                1);
            draw_clock_hand(he, gx, width, height, (st.wSecond *360.0) / 60.0,
                2);
        }

        void draw_clock_hand(HELEMENT he, graphics &gx, UINT sx, UINT sy,
            double angle_degree, int hand)
        {
            dom::element self(he);
            color c(255, 0, 0, 0);
            int hand_width_px;
            ...
            double y = (sy / 2) + 0.5;
                // + 0.5 is to move it to the center of the pixel.
            double x = (sy / 2) + 0.5;
            ...
            if (hand == 2)
            {
                ...
                if (pimage)
                {
                    int w = pimage->width();
                    int h = pimage->height();
                    gx.draw_image(pimage, xe - double(w) / 2, ye - double(h) /
                        2, w, h, 0, 0, w, h);
                }
                else
                // no image
        ...
        void draw_caption(HELEMENT he, graphics &gx, UINT width, UINT height,
            aux::wchars text)
        {
            gx.state_save();
            ...
            gx.no_line();
            gx.fill_linear_gradient(width / 3, 0, (2 *width) / 3, 0, color(0xDF,
                0, 0), color(0, 0, 0x7F));
