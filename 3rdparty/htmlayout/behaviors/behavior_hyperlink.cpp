...
namespace htmlayout
{
    ...
    struct hyperlink: public behavior
    {
        ...
        // ctor
        hyperlink(): behavior(HANDLE_MOUSE | HANDLE_FOCUS | HANDLE_KEY |
            HANDLE_METHOD_CALL, "hyperlink"){}
            ...
            POINT pt, UINT mouseButtons, UINT keyboardStates)
        {
            switch (event_type)
            {
                ...
                case MOUSE_DOWN:
                    ...
                    {
                        dom::element el = he;
                        el.set_state(STATE_CURRENT);
                            // we are reusing STATE_CURRENT to detect click in the element
                        return true;
                    }
                    break;
                case MOUSE_UP:
            ...
            code, UINT keyboardStates)
        {
            if (event_type == KEY_UP)
            {
                if (code != ' ' && code != VK_RETURN)
                    return FALSE;
                dom::element el = he;
                el.post_event(HYPERLINK_CLICK, BY_KEY_CLICK, el);
                    // send bubbling event.
                notify(el, NMHL_HYPERLINK::CLICK);
                    // send standard notifiaction too.
                return TRUE;
            }
            return FALSE;
        }
        ...
        virtual BOOL on_method_call(HELEMENT he, UINT methodID, METHOD_PARAMS
            *params)
        {
        ...
        void notify(dom::element &el, NMHL_HYPERLINK::type code)
        {
            ...
            if (pHREF)
            {
                if (code == NMHL_HYPERLINK::CLICK && pHREF[0] == '#')
                // anchor name, this is a local hyperlink
                {
                    if (pHREF + 1 == 0)
                    // href='#' case
                        return ;

                    dom::element anchor_el = root.find_first(
                        "[id='%S'],[name='%S']", pHREF + 1, pHREF + 1);
                    //find_element_by_name(el.root_element(hwnd), pHREF + 1);
                    if (anchor_el.is_valid())
                    // found
                    {
                        anchor_el.scroll_to_view(true /* scroll it to top of
                            the view */);
            ...
            if (pszTarget)
            {
                if (code == NMHL_HYPERLINK::CLICK && try_to_load(root,
                    nm.szHREF, pszTarget))
                    return ;
        ...
        bool try_to_load(dom::element &root, const wchar_t *url, const wchar_t
            *target)
        {
            if (target[0] == '_')
                return false;
            // do ShellExecute here to open default browser if needed.
