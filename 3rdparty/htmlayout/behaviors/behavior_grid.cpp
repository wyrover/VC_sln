...
namespace htmlayout
{
    ...
    {
        // ctor
        grid(const char *name = "grid", int additional_flags = 0): behavior
            (HANDLE_MOUSE | HANDLE_KEY | HANDLE_FOCUS | additional_flags, name)
            {}
        ...
        virtual void attached(HELEMENT he)
        {
                ...
                assert(aux::streq(table.get_element_type(), "table"));
                    // must be table.
            #endif
        ...
        virtual void on_column_click(dom::element &table, dom::element
            &header_cell)
        {
            table.post_event(TABLE_HEADER_CLICK, header_cell.index(),
                header_cell);
        }
        ...
        /** set current row **/
        void set_current_row(dom::element &table, dom::element &row, UINT
            keyboardStates, bool dblClick = false, bool smooth = false)
        {
            ...
            if (is_multiple(table))
            {
                if (keyboardStates &SHIFT_KEY_PRESSED)
                {
                    ...
                    check_range(table, row.index(), TRUE);
                        // from current to new
                }
                else
                {
            ...
            {
                if (prev != row)
                    prev.set_state(0, STATE_CURRENT | old_row_checked);
                // drop state flags
            }
            ...
            table.post_event(dblClick ? TABLE_ROW_DBL_CLICK : TABLE_ROW_CLICK,
                row.index(), row);
        }

        dom::element target_row(const dom::element &table, const dom::element
            &target)
        {
        ...
        dom::element target_header(const dom::element &header_row, const dom
            ::element &target)
        {
        ...
        void set_checked_row(dom::element &table, dom::element &row, bool
            toggle = false)
        {
            if (toggle)
            {
            ...
            POINT pt, UINT mouseButtons, UINT keyboardStates)
        {
            ...
            if (event_type != (MOUSE_DOWN | SINKING) && event_type !=
                MOUSE_DCLICK)
                return false;
            ...
            // click on the row
            {
                ...
                set_current_row(table, row, keyboardStates, event_type ==
                    MOUSE_DCLICK);
                HTMLayoutUpdateWindow(table.get_element_hwnd(false));
            }
            return FALSE;
                //mouseButtons == MAIN_MOUSE_BUTTON; // as it is always ours then stop event bubbling
        }

        //int firstVisibleRow( dom::element& table )

        virtual BOOL on_key(HELEMENT he, HELEMENT target, UINT event_type, UINT
            code, UINT keyboardStates)
        {
            if (event_type == KEY_DOWN)
            {
                ...
                switch (code)
                {
                    ...
                    case VK_PRIOR:
                        {
                            RECT trc = table.get_location(ROOT_RELATIVE |
                                SCROLLABLE_AREA);
                            int y = trc.top - (trc.bottom - trc.top);
                            int first = fixed_rows(table);
                            dom::element r;
                            for (int i = table.children_count() - 1; i >=
                                first; --i)
                            {
                                dom::element nr = table.child(i);
                                if (aux::wcseq(nr.get_style_attribute("display")
                                    , L"none"))
                                    continue;
                                dom::element pr = r;
                                r = nr;
                                if (r.get_location(ROOT_RELATIVE | BORDER_BOX)
                                    .top < y)
                                {
                                    // row found
                                    if (pr.is_valid())
                                        r = pr;
                                    // to last fully visible
                                    break;
                                }
                            }
                            set_current_row(table, r, keyboardStates, false,
                                true);
                        }
                        return TRUE;
                    ...
                    case VK_END:
                        ...
                        return TRUE;
                    case 'A':
                        if (is_multiple(table) && (keyboardStates
    ...
    struct sortable_grid: public grid
    {
        ...
        struct row_sorter: public dom::element::comparator
        {
            ...
            virtual int compare(const htmlayout::dom::element &r1, const
                htmlayout::dom::element &r2)
            {
