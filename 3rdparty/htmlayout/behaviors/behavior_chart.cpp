...
namespace htmlayout
{
    ...
    struct chart: public canvas
    {
        ...
        // ctor
        chart(): canvas(HANDLE_TIMER | HANDLE_BEHAVIOR_EVENT |
            HANDLE_METHOD_CALL, DRAW_CONTENT)
        {
            step = 0;
            steps = 32;
        }
        ...
        virtual void attached(HELEMENT he)
        {
            ...
            if (data_selector)
            {
                ...
                if (del.is_valid())
                    ...
                    // if it is <script type="application/json"> then the value is parsed automatically.
            }
        }
        ...
        virtual BOOL on_script_call(HELEMENT he, LPCSTR name, UINT argc, json
            ::value *argv, json::value &retval)
        {
            if (aux::streq(name, "animate"))
            {
                if (argc == 1 && argv[0].is_int())
                // we accept single parameter - n - number of steps.
                    steps = argv[0].get(32);
        ...
        virtual void draw_message(graphics &gx, UINT width, UINT height, aux
            ::wchars text)
        {
        ...
        virtual void draw(HELEMENT he, graphics &gx, UINT width, UINT height)
        {
            ...
            gx.line_color(black);
            gx.line(0.5, height - 0.5, width - 0.5, height - 0.5);
                // 0.5 - to draw line in the middle of the pixel
            ...
            for (int n = 0; n < data.length(); ++n)
            {
                ...
                {
                    draw_message(gx, width, height, const_wchars(
                        "Bad data structure"));
                    return ;
                }
                draw_bar(gx, width, height, n, data.length(), color(color_v.get
                    (0)), value_v.get(1.0));
            }
        }

        void draw_bar(graphics &gx, UINT width, UINT height, int n_bar, int
            total_bars, color c, double val)
        {
            double bar_width = double(width) / (2 *total_bars);
            double bar_height = double(height - 2) *val *(double(step) / double
                (steps - 1));
