...
namespace htmlayout
{
    ...
    struct shellicon: public behavior
    {
        ...
        virtual BOOL on_draw(HELEMENT he, UINT draw_type, HDC hdc, const RECT
            &rc)
        {
            ...
            SHFILEINFOW sfi;
            #ifdef _WIN32_WCE
                HIMAGELIST hlist = (HIMAGELIST)SHGetFileInfo(filename, 0, &sfi,
                    sizeof(SHFILEINFO), SHGFI_USEFILEATTRIBUTES |
                    SHGFI_SYSICONINDEX | SHGFI_SMALLICON);
            #else
                HIMAGELIST hlist = (HIMAGELIST)SHGetFileInfoW(filename, 0, &sfi,
                    sizeof(SHFILEINFO), SHGFI_USEFILEATTRIBUTES |
                    SHGFI_SYSICONINDEX | SHGFI_SMALLICON);
            #endif
            if (!hlist)
